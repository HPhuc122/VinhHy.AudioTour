...
export function PoiFormModal({ open, onClose, onSubmit, loading, editPoi }: Props) {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Button onClick={form.handleSubmit(handleSubmit as any)} loading={createMutation.isLoading || updateMutation.isLoading}>
            {createMutation.isLoading || updateMutation.isLoading ? 'Đang lưu...' : isEdit ? 'Lưu thay đổi' : 'Tạo địa điểm'}
          </Button>
        </>
      }
    >
