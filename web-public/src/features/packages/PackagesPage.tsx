...
export function PackagesPage({ lang }: { lang: Lang }) {
  ...
  const [states, setStates] = useState<Record<string, PackageState>>(() => hydratePackageStates());

  const setPackageState = useCallback((code: string, next: Partial<PackageState>) => {
    setStates((current) => ({
      ...current,
      [code]: {
        ...current[code],
  ...
  const refreshStoredPasses = useCallback(() => {
    setStates((current) => {
      const next = { ...current };
      for (const access of guestAccessStore.getAllActive()) {
        next[access.qrCode] = {
          ...next[access.qrCode],
          isProcessing: false,
          error: null,
  ...
  useEffect(() => {
    ...
    const handleStorageChanged = () => refreshStoredPasses();
    const handlePageShown = () => refreshStoredPasses();
    const handleFocus = () => refreshStoredPasses();
  ...
  useEffect(() => {
    ...
    void Promise.allSettled(storedPasses.map(async (storedPass) => {
      try {
        ...
        const currentPass = guestAccessStore.get(storedPass.qrCode);
        if (currentPass?.accessToken !== storedPass.accessToken) {
          return;
        }
        ...
        if (!cancelled) {
          setPackageState(storedPass.qrCode, { access: storedPass });
        }
      }
    }));
  ...
  const purchaseMutation = useMutation({
    mutationFn: async (pkg: PublicPackageDto) => {
      ...
      const started = await publicAccessApi.start(pkg.code, lang);
      let access: GuestAccessRecord;
      ...
      if (!started.requiresPayment && started.accessToken && started.expiresAt) {
        ...
        const paid = await publicAccessApi.simulatePayment(started.paymentSessionId, true, lang);
      ...
      setPackageState(access.qrCode, { access });
      return access;
    },
    onSuccess: (access) => {
      setPackageState(access.qrCode, { access });
    ...
    onError: (_, pkg) => {
      setPackageState(pkg.code, { error: t('cannotPlay') });
    },
  });
  ...
  return (
    <main className="min-h-screen bg-gray-950 px-4 py-24 text-white">
      <div className="mx-auto max-w-6xl">
        <div className="mb-8">
          <p className="mb-2 text-sm font-medium text-emerald-300">KhanhHoi AudioTour</p>
          <h1 className="text-3xl font-bold">{t('passesTitle')}</h1>
          <p className="mt-3 max-w-2xl text-sm text-gray-300">{t('passesText')}</p>
        </div>

        {packagesQuery.isLoading ? (
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">{t('loadingPasses')}</div>
        ) : null}

        {packagesQuery.error || (!packagesQuery.isLoading && !packages.length) ? (
          <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">{t('noPasses')}</div>
        ) : null}
...
function hydratePackageStates(): Record<string, PackageState> {
  return Object.fromEntries(
    guestAccessStore.getAllActive().map((access) => [
      access.qrCode,
      {
        isProcessing: false,
        error: null,
...
function PackageCard({
  ...
  return (
    <article className="rounded-2xl border border-gray-800 bg-gray-900 p-6 shadow-xl">
      <div className="mb-5">
        <p className="text-xs font-medium uppercase text-emerald-300">{pkg.requiresPayment ? t('pay') : t('free')}</p>
        <h2 className="mt-2 text-xl font-semibold">{t('passesTitle')}</h2>
        <p className="mt-2 text-sm text-gray-400">{pkg.code}</p>
      </div>

      <div className="mb-5 flex items-end gap-2">
        <span className="text-3xl font-bold">{pkg.requiresPayment ? formatCurrency(pkg.priceAmount, lang) : t('free')}</span>
        <span className="pb-1 text-sm text-gray-400">/ {pkg.accessDurationMinutes} {t('minutes')}</span>
      </div>

      <ul className="mb-6 space-y-2 text-sm text-gray-300">
        <li>{t('passBenefitArea')}</li>
        <li>{t('passBenefitPoi')}</li>
        <li>{t('passBenefitLanguage')}</li>
      </ul>

      {access ? (
        <div className="space-y-3">
          <AccessCountdown expiresAt={access.expiresAt} onExpired={onExpired} />
          <Link to={ROUTES.TOURS} className="block rounded-xl bg-emerald-600 py-3 text-center text-sm font-medium text-white">
            {t('startListening')}
          </Link>
        </div>
      ) : (
      ...
      {state?.error ? (
        <div className="mt-4 rounded-xl bg-red-500/10 px-4 py-3 text-sm text-red-100">{state.error}</div>
      ) : null}
    </article>
  );
}
