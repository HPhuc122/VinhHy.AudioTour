...
export function PackagesPage({ lang }: { lang: Lang }) {
  ...
  return <main className="min-h-screen bg-gray-950 px-4 py-24 text-white"><div className="mx-auto max-w-6xl"><div className="mb-8"><p className="mb-2 text-sm font-medium text-emerald-300">KhanhHoi AudioTour</p><h1 className="text-3xl font-bold">{t('passesTitle')}</h1><p className="mt-3 max-w-2xl text-sm text-gray-300">{t('passesText')}</p></div>{packagesQuery.isLoading ? <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">{t('loadingPasses')}</div> : null}{packagesQuery.error || (!packagesQuery.isLoading && !packages.length) ? <div className="rounded-2xl border border-gray-800 bg-gray-900 p-6">{t('noPasses')}</div> : null}<div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">{packages.map((pkg) => <PackageCard key={pkg.code} pkg={pkg} lang={lang} state={states[pkg.code]} onBuy={() => purchaseMutation.mutate(pkg)} onExpired={() => { guestAccessStore.remove(pkg.code); setPackageState(pkg.code, { access: null }); }} />)}</div></div></main>;
}
